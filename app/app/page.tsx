...
export default function Home() {
  ...
  // Silent Mode - disables WhatsApp confirmations (for bulk digitization)
  ...
  // Centralized refresh — called after any mutation
  // Refreshes the current view AND clears stale data from the other view
  const refreshCurrentView = useCallback(async () => {
    if (viewMode === 'week') {
      const days = getWeekDays(selectedDate);
      await fetchWeekAppointments(days);
      // Clear day view so it re-fetches fresh when switching
      setAppointments([]);
    } else {
      await fetchAppointments(selectedDate);
      // Clear week view so it re-fetches fresh when switching
      setWeekAppointments({});
    }
  }, [viewMode, selectedDate, fetchAppointments, fetchWeekAppointments]);
        ...
        <div className="bg-amber-50 dark:bg-amber-900/30 border-b border-amber-200 dark:border-amber-800 px-4 py-2 flex items-center justify-between gap-3 animate-fade-in">
          <div className="flex items-center gap-2 text-amber-800 dark:text-amber-200">
            ...
            <span className="text-sm font-medium">
              <strong>Modo Silencioso</strong> — No se enviarán confirmaciones por WhatsApp al crear citas
            </span>
          </div>
          <button
            onClick={() => setSilentMode(false)}
            className="text-xs px-3 py-1 bg-amber-200 dark:bg-amber-800 text-amber-900 dark:text-amber-100 rounded-full font-medium hover:bg-amber-300 dark:hover:bg-amber-700 transition-colors whitespace-nowrap"
          >
            Desactivar
          </button>
        </div>
